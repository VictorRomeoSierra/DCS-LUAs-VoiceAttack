...
csar.csarUnits = {
    ...
    "MEDEVAC #15",
    "MEDEVAC #16",
	"unit|2|190|24|UH-1H| Pilot #1",
	"unit|2|190|24|UH-1H| Pilot #2",
	"unit|2|255|1001|UH-1H| Pilot #1",
	"unit|2|256|1001|UH-1H| Pilot #1",
	"unit|2|254|24|UH-1H| Pilot #1",
	"unit|2|254|24|UH-1H| Pilot #2",
  ...
  csar.bluemash = {
    "FARP Alpha_helipad_0",
    "FARP Bravo_helipad_0",
    "FARP Charlie_helipad_0",
    "FARP Delta_helipad_0",
    "FARP Echo_helipad_0",
    "FARP Foxtrot_helipad_0",
    "FARP Golf_helipad_0",
    "FARP Hotel_helipad_0",
    "FARP India_helipad_0",
    "FARP Juliet_helipad_0",
    "FARP Kilo_helipad_0",
    "FARP Mike_helipad_0",
    "BlueMASH #10"
...
csar.numRescues = 0
csar.valuePerRescueMil = 5
csar.valueTotalRescues = 0


-----------MODIFIED FROM veafTransportMission.lua FROM VEAF-------------------------
-- automatically add all the human-manned transport helicopters to ctld.transportPilotNames

function csar.initializeAllHelosInCSAR()
    local TransportHeloTypeNames = {"Mi-8MT", "UH-1H", "AH-6J", "Mi-24P", "AH-64D_BLK_II", "Mi-24P Hind F", "Bronco-OV-10A"}
    local result = {}
	local units = mist.DBs.humansByName -- local copy for faster execution
        ...
        if unit then
			for _, transportHeloTypeName in pairs(TransportHeloTypeNames) do
				if transportHeloTypeName:lower() == unit.type:lower() then
					table.insert(csar.csarUnits, unit.unitName)
...
csar.maxLives = 3 -- Maximum pilot lives
...
csar.destructionHeight = 10 -- height in meters an aircraft will be destroyed at if the aircraft is disabled
...
function csar.eventHandler:onEvent(_event)
    local status, err = pcall(function(_event)
        ...
        --[[elseif (_event.id == 9) then
            ...
            return
        --]]
        elseif world.event.S_EVENT_EJECTION == _event.id or (_event.id == 9) then
            ...
            trigger.action.outTextForCoalition(_unit:getCoalition(), "MAYDAY MAYDAY! " .. _unit:getTypeName() .. " shot down.", 10)
...
function csar.checkCloseWoundedGroup(_distance, _heliUnit, _heliName, _woundedGroup, _woundedGroupName)
    ...
    if csar.heliVisibleMessage[_lookupKeyHeli] == nil then
        ...
        csar.displayMessageToSAR(_heliUnit, string.format("%s: %s. I hear you! Land or hover by the smoke.", _heliName, _pilotName), 30)
    ...
    if (_distance < 500) then
        ...
        if csar.inAir(_heliUnit) == false then
            ...
            if csar.inAir(_heliUnit) and _unitsInHelicopter + 1 <= csar.max_units then
                ...
                if _distance < 12.0 then

                    --check height!
                    local _height = _heliUnit:getPoint().y - _woundedLeader:getPoint().y

                    if _height <= 100.0 then --max height - maxheight
...
function csar.scheduledSARFlight(_args)
    ...
    local _status, _err = pcall(function(_args)
        ...
        --if _dist < 1500 then
            --csar.displayMessageToSAR(_heliUnit, string.format("%s: MASH, CSAR on final, Prep for landing...", _heliUnit:getName()), 10)
        --end

        if _dist < 1000 and _heliUnit:inAir() == false then
            csar.displayMessageToSAR(_heliUnit, string.format("%s: Get the stretcher!...", _heliUnit:getName()), 10)
...
function csar.rescuePilots(_heliUnit)
    ...
    local _txt = string.format("%s: The pilots have been taken to the\nmedical clinic. Good job! (+1 to weapons load)", _heliUnit:getName())
    if WeaponPoints.MaxPoints["Air"] ~= nil then
      if WeaponPoints.MaxPoints["Air"] < 10 then
        WeaponPoints.MaxPoints["Air"] = WeaponPoints.MaxPoints["Air"] + 1
      end
      if WeaponPoints.MaxPoints["Ground"] < 10 then
        WeaponPoints.MaxPoints["Ground"] = WeaponPoints.MaxPoints["Ground"] + 1
      end
    end
    -- enable pilots again
    for _, _rescueGroup in pairs(_rescuedGroups) do
    ...
    csar.numRescues = csar.numRescues + 1
    csar.valueTotalRescues = (csar.numRescues * csar.valuePerRescueMil)

    csar.displayMessageToSAR(_heliUnit, _txt, 10)
    --csar.displayMessageToSAR(_heliUnit, "Rescued " .. csar.numRescues .. " for $" .. csar.valueTotalRescues .. "M", 10)
    trigger.action.outTextForCoalition(_heliUnit:getCoalition(), "Rescued " .. csar.numRescues .. " for $" .. csar.valueTotalRescues .. "M", 10)
    --hammer
...
function csar.getSARHeli(_unitName)
    ...
    if _heli ~= nil and _heli:getLife() ~= nil then
      if _heli:getLife() > 0 then
...
function csar.getWoundedGroup(_groupName)
    local _status, _result = pcall(function(_groupName)
        ...
        for _, _unit in pairs(_units) do
            ...
            if _unit ~= nil and _unit:getLife() ~= nil then
...
function csar.getClosetMASH(_heli)
    ...
    for _, _mashName in pairs(_mashes) do
        ...
        if _mashUnit ~= nil and _mashUnit:getLife() ~= nil then
          if _mashUnit:getLife() > 0 then

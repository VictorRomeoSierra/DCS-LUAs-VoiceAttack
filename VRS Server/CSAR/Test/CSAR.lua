...
csar.csarFixedUnits =  {
    "BLUE PLAYER UH-1 ROME",
    "BLUE PLAYER UH-1H ROME",
    "BLUE PLAYER MI-8 ROME",
    "BLUE PLAYER MI-8 MADRID",
    "BLUE PLAYER UH-1H MADRID",
    "BLUE PLAYER UH-1H 2 MADRID",
    "RED PLAYER MI-8 MOSCOW",
    "RED PLAYER MI-8 2 MOSCOW",
    "BLUE PLAYER UH-1 DUBLIN",
    "BLUE PLAYER UH-1 2 DUBLIN",
    "BLUE PLAYER UH-1 3 DUBLIN",
    "BLUE PLAYER UH-1 4 DUBLIN",
    "BLUE PLAYER MI-8 DUBLIN",
    "BLUE PLAYER MI-8 2 DUBLIN",
    "RED PLAYER MI-8 2 TORBA",
    "RED PLAYER MI-8 TORBA",
    "BLUE PLAYER MI-24 HOMEBASE-2",
    "BLUE PLAYER MI-24 HOMEBASE-1",
    "BLUE PLAYER MI-24 HOMEBASE",
    "RED PLAYER MI-24 MOSCOW-2",
    "RED PLAYER MI-24 MOSCOW-1",
    "RED PLAYER MI-24 MOSCOW 1",
    "RED PLAYER MI-24 TORBA-1",
    "RED PLAYER MI-24 TORBA",
    "BLUE PLAYER MI-24 MADRID-2",
    "BLUE PLAYER MI-24 MADRID-1",
    "BLUE PLAYER MI-24 MADRID 1",
    "BLUE PLAYER MI-24 DUBLIN 3",
    "BLUE PLAYER MI-24 DUBLIN 2",
    "BLUE PLAYER MI-24 DUBLIN 1",
    "helicargo1",
    "helicargo2",
    "helicargo3",
    "helicargo4",
...
csar.maxLives = 1 -- Maximum pilot lives
...
function csar.pickupUnit(_heliUnit, _pilotName, _woundedGroup, _woundedGroupName)
	...
	CaucasusPendulumScore:ScoreCSV( csar.getPlayerNameOrType(_heliUnit), "", "CSAR_PICKUP", 1, 50, _heliUnit:getName() )
...
function csar.getPlayerNameOrType(_heli)
...
function csar.rescuePilots(_heliUnit)
    ...
    -- enable pilots again
    for _, _rescueGroup in pairs(_rescuedGroups) do
		CaucasusPendulumScore:ScoreCSV( csar.getPlayerNameOrType(_heliUnit), "", "CSAR_SUCCESS", 1, 100, _heliUnit:getName() )
		CaucasusPendulum.addResources(_heliUnit:getCoalition(), 1)
...
function csar.displayActiveSAR(_unitName)
    ...
    for _groupName, _value in pairs(csar.woundedGroups) do
        ...
        if #_woundedGroup > 0 and (_woundedGroup[1]:getCoalition() == _heliSide) and (type(_value.frequency) == "number") then
...
function csar.toggleOnDuty(_unitName)
    local _heliName = csar.getSARHeli(_unitName)
    local _groupId = csar.getGroupId(_heliName)
    
    if _groupId then
        if csar.onDutyList[_groupId] == nil or csar.onDutyList[_groupId] == false then
            csar.onDutyList[_groupId] = true
            csar.displayMessageToSAR(_heliName, "You reported for CSAR duty!", 30)
        elseif csar.onDutyList[_groupId] == true then
            csar.onDutyList[_groupId] = false
            csar.displayMessageToSAR(_heliName, "You are now off CSAR duty.", 30)
        end
    end
end

function csar.onDutyCheck(_heliName)
    local _groupId = csar.getGroupId(_heliName)
    
    if _groupId then   
        if csar.onDutyList[_groupId] ~= nil and csar.onDutyList[_groupId] == true then
            return true
        end
    end

    csar.displayMessageToSAR(_heliName, "You are not on CSAR duty.", 30)
    return false
end
...
function csar.signalFlare(_unitName)
    ...
    if _heli == nil or not csar.onDutyCheck(_heli) then
        return
    end
...
function csar.checkOnboard(_unitName)
    ...
    if _unit == nil or not csar.onDutyCheck(_unit) then
        return
    end
        ...
        if _unit ~= nil then
            ...
            if _groupId then
                ...
                if csar.addedTo[tostring(_groupId)] == nil then
                    ...
                    missionCommands.addCommandForGroup(_groupId, "Report On Duty", _rootPath, csar.toggleOnDuty, _unitName)
